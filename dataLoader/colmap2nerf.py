...
def parse_args():
    ...
    parser.add_argument("--masks", help="[Optional] input path to the image masks")
...
def run_colmap(args):
    ...
    colmap_cmd = f"colmap feature_extractor --ImageReader.camera_model OPENCV --SiftExtraction.estimate_affine_shape=true --SiftExtraction.domain_size_pooling=true --ImageReader.single_camera 1 --database_path {db} --image_path {images}"
    if args.masks:
        colmap_cmd += f" --ImageReader.mask_path {args.masks}"
    do_system(colmap_cmd)
